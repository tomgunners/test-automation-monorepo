...
import fs from 'fs';
import { setupMobileAllure } from '../utils/allure-setup';
...
export const sharedConfig: Partial<Options.Testrunner> = {
   ...
   *
   * Ordem das operações:
   *   1. clearAllureResults() — apaga resultados anteriores
   *   2. setupMobileAllure() — restaura history e escreve metadados
  ...
  async afterTest(test, _ctx, { passed, error }) {
    ...
    try {
      ...
      console.error(
        '[afterTest] Falha ao capturar screenshot:',
        screenshotErr,
      );
    }
    ...
    try {
      ...
      console.error(
        '[afterTest] Falha ao registrar log no Allure:',
        logErr,
      );
    }
  },
  ...
  onComplete(_exitCode, _config, _caps, results) {
    ...
    console.log(
      '  Execute: [yarn test:mobile:allure] → relatório Allure',
    );
    console.log('════════════════════════════════════════════════\n');
  },
};