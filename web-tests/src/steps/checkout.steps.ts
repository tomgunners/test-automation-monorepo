...
When('clico em "Continue"', async function (this: CustomWorld) {
  await await this.checkout.clickContinue();
});
...
When('clico em "Finish"', async function (this: CustomWorld) {
  await await this.checkout.clickFinish();
});

// ─── Then ─────────────────────────────────────────────────────────────────────

Then('devo ver a tela de confirmação do pedido', async function (this: CustomWorld) {
  await await this.checkout.waitForConfirmation();
  const isVisible = await await this.checkout.isConfirmationVisible();
...
Then(
  ...
  async function (this: CustomWorld, expectedMessage: string) {
    const header = await await this.checkout.getConfirmationHeader();
...
Then(
  ...
  async function (this: CustomWorld) {
    const isErrorVisible = await await this.checkout.isErrorVisible();
    expect(isErrorVisible).toBeTruthy();

    const errorMessage = await await this.checkout.getErrorMessage();
